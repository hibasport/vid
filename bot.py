...
def clean_title(raw):
    ...
    # 3) كلمة لاتينية قصيرة في البداية + نقطة أو -
    t = re.sub(r'^[a-zA-Z0-9._@-]{2,20}\s*[.\-:]\s+', '', t).strip()

    # 4) - كلمة_لاتينية_قصيرة في النهاية
...
def get_font():
    ...
    if os.path.exists(local):
        print("✅ Font: Montserrat-Arabic-Bold (local)")
        return local
    dv = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
    print("⚠️ Using fallback font: DejaVu")
    return dv
...
def apply_green_screen(main, gs, out, w, h, dur):
    ...
    r = subprocess.run(
        ["ffmpeg", "-y", "-i", main, "-i", gs, "-filter_complex",
         f"[1:v]trim=duration={dur},scale={w}:{h},colorkey=0x00FF00:0.3:0.1,setpts=PTS-STARTPTS[g];"
...
for source in config["sources"]:
    try:
        all_videos.extend(get_videos_from_source(source))
    except Exception as e:
...
if not new_video:
    ...
    if not download_video(new_video["url"]):
        exit(1)
    ...
    if src_w == TARGET_W and src_h == TARGET_H:
        ...
        print("  ✅ المقاس مطابق، لا حاجة لتحجيم")
    else:
